...
* 范围查询：当`select * from T1 where b > 12 and c > 14 and a > 3`时，mysql只会根据b > 12查找到数据
  ...
  之所以会有最左前缀匹配原则和联合索引的**索引构建方式及存储结构**是有关系的。首先我们创建的idx_t1_bcd(b,c,d)索引，**相当于创建了(b)、（b、c）（b、c、d）三个索引**，而在范围查询b>12的时候其实只用到了（b）这一个索引
...
Mysql为每个client都维护了一个binlog cache，当要写入bin log的时候先写入到binlog cache中，通过sync_binlog参数来判断多久把binlog cache中的数据写入到磁盘，sync_binlog取值范围是 0-N
* 0：不去强制要求，由系统自行判断何时写入磁盘
* 1：每次 commit 的时候都要将 binlog 写入磁盘
* N：每N个事务，才会将 binlog 写入磁盘

如果仅仅依靠bin log恢复是会有数据丢失的风险，一是因为sync_binlog设置为0或大于1的数的时候，binlog数据记录不全，二是因为innodb内部存在redo log，加上redo log才能保证数据恢复准确，bin log作用仅仅只是归档，并不保证数据的持久性
...
> 不论获取到的是什么锁，当前读获取到的锁不会因为当前读语句被执行而释放，锁的释放会等到事务提交之后才释放，这就是锁的两阶段释放（下面会讲）
...
* 加锁的最小范围是Next-key lock，Next-key lock是前开后闭的
* 在RC级别及以下，Next-key lock会退化成行锁
