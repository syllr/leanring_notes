...
- 并且**同一个ProducerID**对同一个分区只能**顺序的发送消息**

这样当broker接收到生产者发送的消息时会检查当前内存里面的存的ProducerID对应的SequenceNumber，如果消息的SequenceNumber小于等于broker中存的最大值，说明消息是重复的，返回true，但是不对消息持久化。Kafka是利用了消息SequenceNumber顺序递增的特性实现了发送消息的幂等性

Kafka的消息幂等是建立在同一个Producer上的，并且要求同一个Producer对同一个分区发送的消息是串行的，降低了一点吞吐量

而且要保证精确一次性消费，还要保证消费者只消费一次消息，消费者想要实现一次消费需要满足两个条件

* 因为kafka的消费进度是由消费者管理的，消费者每次消费一个消息都进行一次消费进度的提交，不然会因为消费者宕机重启，导致消费进度丢失一部分，发生重复消费
* 可以一次拉取多条消息，然后顺序执行，每个消息执行成功之后提交进度，然后再执行下一条消息，因为kafka有多种进度提交方式，可用根据业务逻辑选择手动提交的方式来决定提交的逻辑
